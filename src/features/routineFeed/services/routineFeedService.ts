...
const routineFeedResponseSchema = z.object({
  posts: z.array(z.object({
    ...
    executed_action_count: z.number().int().nonnegative().optional(),
    ...
    routine_execution_memo: z.string().nullable().optional(),
    support_count: z.number().int().nonnegative().optional(),
...
function toRoutine(post: RoutineFeedResponse['posts'][number]): Routine {
  return routineSchema.parse({
    ...
    executedActionCount: post.executed_action_count,
    executionMemo: post.routine_execution_memo,
    ...
    supportCount: post.support_count,

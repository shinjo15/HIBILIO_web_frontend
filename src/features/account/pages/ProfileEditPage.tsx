...
export function ProfileEditPage({ service = profileEditService }: ProfileEditPageProps) {
  ...
  return <main className="profile-edit">
    ...
    <section className="profile-edit__content">
      ...
      <div className="profile-edit__header-image" style={headerPreviewUrl === null ? undefined : { backgroundImage: `url(${headerPreviewUrl})` }}><label><span>{messages.profileEdit.changeHeader}</span><input accept="image/png,image/jpeg,image/webp" onChange={(event) => selectImage('header', event.target.files?.[0] ?? null)} type="file" /></label></div>
      <div className="profile-edit__avatar" style={iconPreviewUrl === null ? undefined : { backgroundImage: `url(${iconPreviewUrl})` }}><span>{iconPreviewUrl === null && editingProfile.name.slice(0, 1).toUpperCase()}</span><label><span>{messages.profileEdit.changeIcon}</span><input accept="image/png,image/jpeg,image/webp" onChange={(event) => selectImage('icon', event.target.files?.[0] ?? null)} type="file" /></label></div>

...
export function RegisterPage() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (registration.step === 'social') {
      registration.continueToTags();
      return;
    }

    void registration.submitFavoriteTags();
  }
  ...
  function navigateBack(): void {
    ...
    if (registration.step === 'social') {
      registration.returnToProfile();
      return;
    }

    registration.returnToSocialLinks();
  }

  return (
    <main className="hibilio-register">
      <header className="hibilio-register__header">
        <Button aria-label={messages.auth.backToLogin} className="hibilio-register__back" onClick={navigateBack} type="button" variant="text">
          ←
        </Button>
        <h1>{messages.auth.signUpTitle}</h1>
        <div aria-label={messages.auth.signUpTitle} className="hibilio-register__steps">
          {[0, 1, 2, 3, 4].map((index) => <span className={stepIndex >= index ? 'is-active' : ''} key={index} />)}
        </div>
      </header>
      ...
      <section className="hibilio-register__content">
        <div className="hibilio-register__intro">
          <HibilioMark size={56} />
          <h2>{registration.step === 'email' ? messages.auth.accountCreationTitle : registration.step === 'passcode' ? messages.auth.registrationPasscodeTitle : registration.step === 'profile' ? messages.auth.profileSetupTitle : registration.step === 'social' ? messages.auth.socialLinksTitle : messages.auth.favoriteTagsTitle}</h2>
          <p>{registration.step === 'email' ? messages.auth.accountCreationStep : registration.step === 'passcode' ? messages.auth.registrationPasscodeStep : registration.step === 'profile' ? messages.auth.profileSetupStep : registration.step === 'social' ? messages.auth.socialLinksStep : messages.auth.favoriteTagsStep}</p>
        </div>
        ...
        <form className="hibilio-register__form" noValidate onSubmit={handleSubmit}>
          ...
          {registration.step === 'email' && <>
            ...
            <Button className="hibilio-register__submit" disabled={registration.isSubmitting || registration.emailAddress === ''} fullWidth size="large" type="submit" variant="contained">{messages.auth.requestPasscode}</Button>
          ...
          {registration.step === 'passcode' && <>
            ...
            <div aria-label={messages.auth.passcode} className="hibilio-register__passcode-inputs">
              {passcodeDigits.map((digit, index) => <input aria-label={messages.auth.passcodeDigit.replace('{index}', String(index + 1))} autoComplete={index === 0 ? 'one-time-code' : 'off'} inputMode="numeric" key={index} maxLength={6} onChange={(event) => updatePasscodeDigit(index, event)} onKeyDown={(event) => movePasscodeFocusOnBackspace(index, event)} ref={(element) => { passcodeInputReferences.current[index] = element; }} value={digit} />)}
            </div>
            <Button className="hibilio-register__submit" disabled={registration.isSubmitting || registration.passcode.length !== 6} fullWidth size="large" type="submit" variant="contained">{messages.auth.confirmAndContinue}</Button>
            <div className="hibilio-register__passcode-help"><p>{messages.auth.passcodeNotReceived}</p><Button disabled={registration.isSubmitting} onClick={() => void registration.submitEmailAddress()} type="button" variant="text">{messages.auth.resendPasscode}</Button></div>
          ...
          {registration.step === 'profile' && <>
            ...
            <Button className="hibilio-register__submit" disabled={registration.isSubmitting || registration.accountName === '' || registration.userHandle === ''} fullWidth size="large" type="submit" variant="contained">{messages.auth.profileNext}</Button>
          </>}
